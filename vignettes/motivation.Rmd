...
bayes_three_component <- function(zs, n=2000, sigma=1, sigma_prior = 0, effect_sizes = 3*(-1:1)){
  sigma_marginal <- sqrt(sigma^2 + sigma_prior^2)
...
three_component_df_two_reps <- with(three_component_two_reps, tibble(xs=Zs[,1],
                             ys=Zs[,2], 
                             mus=true_mus,
                             bayes_rule = bayes_three_component(Zs[,1], 
                                                sigma=sigma_3c, 
...
bayes_normal_normal <- function(zs, A=1, sigma=1, prior_mu=0){
  lambda <- A/(A+sigma^2)
